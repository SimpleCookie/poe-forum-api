...
function fetchAndSaveSwaggerSpec(retries = 0) {
  return new Promise((resolve, reject) => {
    console.log(
      `📥 Fetching swagger.json from ${SERVER_URL}/swagger/json (attempt ${retries + 1}/${MAX_RETRIES})...`
    )
    ...
    const req = http.get(`${SERVER_URL}/swagger/json`, { timeout: 5000 }, (res) => {
      ...
      if (res.statusCode !== 200) {
        ...
        if (retries < MAX_RETRIES) {
          ...
          setTimeout(
            () =>
              fetchAndSaveSwaggerSpec(retries + 1)
                .then(resolve)
                .catch(reject),
            RETRY_DELAY
          )
...
async function generateApi() {
  try {
    ...
    console.error(
      '💡 Or manually fetch the spec with: curl http://localhost:3000/swagger/json > swagger.json'
    )
    process.exit(1)
  }
}
