  ...
  return rows
    .map((row) => {
      ...
      return {
        ...
        replies: repliesEl.length ? Number(repliesEl.text().trim() || '0') : 0,
      }
    })
    .filter((thread): thread is CategoryThread => thread !== null)
}

export async function crawlCategoryPage(categorySlug: string, pageNumber: number) {
  const url =
    pageNumber === 1
